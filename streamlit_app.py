...
if menu == "📋 SEZNAM VOZIDEL":
    ...
    if not cars_data:
        ...
        for c in cars_data:
            ...
            tabela_aut.append({
                "VIN": c.get("vin"), "Typ": c.get('type', 'Osobní auto'),
                "Značka a model": c.get('brand_model', 'Neznámé'), "SPZ": c.get('reg_number', '—'),
        ...
        id_vyberu = st.dataframe(df_cars, use_container_width=True, hide_index=True, selection_mode="single-row", key="cars_df_selection")
        ...
        if oznacene_radky:
            ...
            if st.button("Uložit servisní záznam", type="primary", use_container_width=True):
                ...
                if v_type != "Přívěs":
                    for c in db["cars"]:
                        if c["vin"] == avin: c["mileage"] = int(r_km)
                save_data(db)
...
# =================================================================
    ...
    with st.form("add_car_form"):
        ...
        if submitted:
            ...
            elif any(c["vin"] == f_vin for c in db["cars"]):
...
# =================================================================
    ...
    with st.form("add_stock_form"):
        ...
        if stock_submitted:
            if not f_pid or not f_name:
                ...
                novy_dil = {"pid": f_pid, "name": f_name, "quantity": int(f_qty), "price": float(f_price)}
                db["stock"].append(novy_dil)
                save_data(db)
    ...
    stock_data = db["stock"]
    if stock_data:
        tabela_skladu = [{"Kód dílu": item.get("pid"), "Název dílu": item.get("name"), "Množství (ks)": item.get("quantity"), "Nákupní cena (Kč)": f"{item.get('price', 0.0)} Kč"} for item in stock_data]
